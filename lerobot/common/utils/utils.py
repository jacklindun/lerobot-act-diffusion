...
def auto_select_torch_device() -> torch.device:
    ...
    # Check for NPU first (Ascend NPU)
    try:
        import torch_npu
    ...
    match try_device:
        case "npu":
            try:
                ...
                if log:
                    logging.info("Using NPU device.")
            except (ImportError, AssertionError):
                if log:
                    logging.warning("NPU not available, falling back to auto-select.")
...
def is_torch_device_available(try_device: str) -> bool:
    ...
    if try_device == "npu":
        ...
        raise ValueError(f"Unknown device {try_device}. Supported devices are: npu, cuda, mps or cpu.")
...
def is_amp_available(device: str):
    if device in ["cuda", "cpu"]:
        ...
        # NPU supports AMP with torch_npu
        try:
            import torch_npu
            return torch_npu.npu.is_available()
        except ImportError:
